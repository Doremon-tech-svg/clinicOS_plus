...
If totally irrelevant, return {"recognized": false}.`
        ...
        // Diagnostic log — confirms Groq call succeeded or why it didn't
        console.log("Groq /command response:", aiResponse.status, aiResponse.ok, JSON.stringify(data).slice(0, 500));
        ...
        if (!aiResponse.ok) {
          ...
          // extra safety: pull out the {...} block in case model adds stray text
          const jsonMatch = raw.match(/\{[\s\S]*\}/);
          if (jsonMatch) raw = jsonMatch[0];
          try {
            aiResult = JSON.parse(raw);
          } catch (parseErr) {
            console.error("Groq JSON.parse failed. Raw content:", raw);
            aiResult = { recognized: false, error: "parse_failed" };
          }
        }
      } catch (err) {
